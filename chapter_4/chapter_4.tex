...
It is easiest to start by assuming the gas is isothermal and that all incoming photons have only enough energy to ionize the gas (13.6 ev). In this way, all gas is heated to the same temperature, and both cross section (due to only a single photon wavelength) and recombination (due to only a single temperature) can be assumed to be constant.
